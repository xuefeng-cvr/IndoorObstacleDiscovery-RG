function [iou_insts,perc_free] = compute_iou(bbox,gt)
...
integral_fs(:,2:end+1) = integral_fs(:,:);
for i = 1:size(bbox,1)
    a3 = integral_fs(bbox(i,4)+1,   bbox(i,3)+1,:) + ...
         integral_fs(bbox(i,2)-1+1, bbox(i,1)-1+1,:) - ...
         integral_fs(bbox(i,4)+1,   bbox(i,1)-1+1,:) - ...
         integral_fs(bbox(i,2)-1+1, bbox(i,3)+1,:);
    a2 = area(i);
    perc_free(1,i) = a3 / a2;
end
