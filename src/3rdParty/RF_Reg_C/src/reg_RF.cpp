...
void regTree(double *x, double *y, int mdim, int nsample, int *lDaughter,
             ...
             double *tgini, int *varUsed,int* subSampIdx);
           ...
           int *nout, int *inbag, int print_verbose_tree_progression)
{
    ...
    oobpair = (doProx && oobprox) ? (int *)mxCalloc(nsample * nsample, sizeof(int)) : NULL;
    ...
    if (doProx)
    {
        ...
        if (testdat)
            zeroDouble(proxts, ntest * (nsample + ntest));
    }
    ...
    int * subSampIdx = (int *)mxCalloc(*sampsize, sizeof(int));
    for (j = 0; j < *nTree; ++j) // 训练若干棵决策树
    {
        //printf("tree num %d\n",j);fflush(stdout);
        ...
        { /* sampling with replacement */
            for (n = 0; n < *sampsize; ++n) // 样本子集数
            {
                xrand = unif_rand();
                k = xrand * nsample; // 被选中的下标
                in[k] = 1;
                yb[n] = y[k];
                for (m = 0; m < mdim; ++m)
                {
        ...
        /* grow the regression tree 建立一个回归树*/
        regTree(xb, yb, mdim, *sampsize, lDaughter + idx, rDaughter + idx,
                ...
                varUsed, subSampIdx);
        /* predict the OOB data with the current tree 对当前的树用OOB数据进行测试*/
        ...
        if (varImp)
        {
            ...
            {
                if (varUsed[mr])
                { /* Go ahead if the variable is used */
...
#ifdef MATLAB
            time(&curr_time);
            mexPrintf("tree num %d created at %s", j, ctime(&curr_time));
    ...
    if (biasCorr)
    { /* bias correction for predicted values */
        for (n = 0; n < nsample; ++n)
        {
            if (nout[n])
    ...
    if (doProx)
    {
        for (n = 0; n < nsample; ++n)
        {
            ...
            {
                prox[nsample * k + n] /= oobprox ? (oobpair[nsample * k + n] > 0 ? oobpair[nsample * k + n] : 1) : *nTree;
               ...
               double *proxMat, int *nodes, int *nodex)
{
    ...
    if (doProx)
        zeroDouble(proxMat, *n * *n); // 保存每棵树对每个样本的预测
    if (*keepPred)
        zeroDouble(allpred, *n * *ntree);
    idx1 = 0;
    idx2 = 0;
    for (i = 0; i < *ntree; ++i)
    {
        ...
        {
            for (j = 0; j < *n; ++j)
                allpred[j + i * *n] = ytree[j];
        }
        /* if desired, do proximities for this round */
        if (doProx)
            computeProximity(proxMat, 0, nodex + idx2, junk,
                             junk, *n);
    ...
    *decsplit = 0.0;
    ...
    for (i = 0; i < mtry; ++i) // 特征数
    {
        ...
        else
        {
            ...
            /* Compute means of Y by category. 利用类别计算Y的均值*/
            for (j = 0; j < lc; ++j)
            {
        ...
        {
            *ubest = ubestt;
            *msplit = kv + 1; // 找到最佳的分割下标
            critmax = critvar;// 最大的分割值
            for (j = ndstart; j <= ndend; ++j)
            {
                ut[j] = xt[j];
            }
                    ...
                    int *nodex)
{
    ...
    for (i = 0; i < nsample; ++i) // 从第一个样本开始
    {
        ...
        { /* go down the tree */
            ...
            if (cat[m] == 1)
            {
                k = (x[m + i * mdim] <= split[k]) ? lDaughter[k] - 1 : rDaughter[k] - 1;
            }
            else
            {
                /* Split by a categorical predictor */
                k = cbestsplit[(int)x[m + i * mdim] - 1 + k * maxcat] ? lDaughter[k] - 1 : rDaughter[k] - 1;
            }
        }
